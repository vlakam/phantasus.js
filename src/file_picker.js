phantasus.FilePicker = function (options) {
  ...
  this.$el = $el;
  for (var i = 0; i < options.addOn.length; i++) {
    var $div = $el.find('#' + options.addOn[i].id + ' > .phantasus-landing-panel');
  ...
  $google.on('click', function () {
    ...
    function pickerCallback(data) {
      if (data.action == google.picker.Action.PICKED) {
        ...
        //     var ext = phantasus.Util.getExtension(fileName);
        //     var datasetReader = phantasus.DatasetUtil.getDatasetReader(ext, {interactive: true});
        //     if (datasetReader == null) {
        //       datasetReader = new phantasus.Array2dReaderInteractive();
        //     }
